...
def mnist_tutorial_jsma(train_start=0, train_end=60000, test_start=0,
    ...
    img_rows = int(X_train.shape[1])
    img_cols = int(X_train.shape[2])
    ...
    numAttacked=0
    itr_ind = 0
    alreadyAttacked = [0]*10
    # for sample_ind in xrange(0, source_samples):
    while numAttacked < source_samples:
        print('--------------------------------------')
        print('Attacking input %i/%i' % (itr_ind + 1, source_samples))
        sample = X_test[itr_ind:(itr_ind+1)]

        # We want to find an adversarial example for each possible target class
        # (i.e. all classes that differ from the label given in the dataset)
        current_class = int(np.argmax(Y_test[itr_ind]))
        if 1 == alreadyAttacked[current_class]:
            itr_ind += 1
            continue
        alreadyAttacked[current_class] = 1
        ...
        for target in target_classes:
            ...
            test_in_reshape = X_test[itr_ind].reshape(-1)
            ...
            results[target, numAttacked] = res
            perturbations[target, numAttacked] = percent_perturb
